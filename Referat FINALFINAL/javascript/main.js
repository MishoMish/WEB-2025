...
(function() {
    ...
    function applySVGColors(svg, colors) {
        ...
        backgrounds.forEach(el => {
            ...
            if (fill && (fill.includes('var(--svg-bg)') || fill === '#f8f9fa' || fill === '#1a1a2e' || 
                        (el.getAttribute('width') === svg.getAttribute('viewBox')?.split(' ')[2]))) {
                el.setAttribute('fill', colors.bg);
            } else if (fill && (fill.includes('var(--svg-panel-bg)') || fill === '#ecf0f1' || fill === '#16213e' || fill === 'white')) {
        ...
        texts.forEach(el => {
            ...
            if (fill && !fill.startsWith('#3') && !fill.startsWith('#e') && !fill.startsWith('#9') && 
                !fill.startsWith('#2') && !fill.startsWith('url')) {
                return;
            }
            if (fill === '#2c3e50' || fill === '#e0e0e0' || fill === '#7f8c8d' || fill === '#95a5a6' || fill.includes('var(--svg-text)')) {
        ...
        lines.forEach(el => {
            ...
            if (stroke && (stroke === '#bdc3c7' || stroke === '#95a5a6' || stroke === '#555' || stroke.includes('var(--svg-line)'))) {
                el.setAttribute('stroke', colors.line);
            } else if (stroke && (stroke === '#2c3e50' || stroke === '#e0e0e0' || stroke.includes('var(--svg-text)'))) {
                el.setAttribute('stroke', colors.text);
            } else if (stroke && (stroke === '#e0e0e0' || stroke === '#ecf0f1' || stroke === '#333' || stroke.includes('var(--svg-grid)'))) {
        ...
        rectsWithStroke.forEach(el => {
            const stroke = el.getAttribute('stroke');
            if (stroke && (stroke === '#bdc3c7' || stroke === '#444' || stroke.includes('var(--svg-border)'))) {
                el.setAttribute('stroke', colors.border);
            } else if (stroke && (stroke === '#e0e0e0' || stroke === '#333' || stroke.includes('var(--svg-grid)'))) {
                el.setAttribute('stroke', colors.grid);
            }
        });
    }
